...
At the server side, two general categories of XSS countermeasures exist. The first one is that of \emph{input validation}, or \emph{filtering}, as is available in solutions like Sanctum's AppShield \cite{Klein2002}. In such an approach, all script code is stripped from user input, in order to prevent any user-submitted executable code to be sent to the browser. This is done either by removing certain substrings (e.g. \texttt{<script>} tags), or by replacing characters with their HTML-safe counterparts (e.g. replacing \texttt{\begin{verbatim}<\end{verbatim}} by \texttt{\begin{verbatim}&lt;\end{verbatim}} and \texttt{\begin{verbatim}>\end{verbatim}} by \texttt{\begin{verbatim}&gt;\end{verbatim}}), which will be converted back to the original character once they are rendered by the browser. A difficulty of such an approach is that it has to be able to recognize all different ways in which script code can be injected \cite{Jim2007}, including all possible encodings and taking into account browser quirks for different browsers \cite{Bisht2008}.

A second category of server-side XSS countermeasures compares a version of the web page that does not contain user input to a version that does, and makes sure that the script code in both versions is the same. For example, XSS-Guard, developed by Bisht et al. \cite{Bisht2008}

\subsection{Client-side countermeasures}
% Vertellen dat geen van de oplossingen voldoende zijn om volledig te beschermen tegen XSS aanvallen
