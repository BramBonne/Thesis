...
\newglossaryentry{attack vector}{name=attack vector,description={A method which can be used to successfully execute a certain attack}}
\newglossaryentry{csrf}{name=CSRF,description={Cross-Site Request Forgery, an attack in which the attacker leverages the victim's browser's implicit authentication to make requests in the name of the victim}}
...
\newglossaryentry{tld}{name=TLD,description={Top Level Domain, the last part of a \gls{url}, which determines the part of the domain that is registered in the name space's root zone. Examples are \texttt{.com}, \texttt{.org} and \texttt{.co.uk}}}
...
\newglossaryentry{phishing}{name=phishing,description={An attack wherein a victim is tricked into thinking the attacker is a trusted party}}
\newglossaryentry{psid}{name=PSID,description={Proxy Session Identifier, a second \gls{session id} which is attached and checked by a proxy in order to counter session fixation attacks}}
...
\newglossaryentry{login session fixation}{name=login session fixation,description={A popular variant of the \gls{session fixation} attack, in which an attacker tricks the user into logging in with a predefined \gls{sid}}}
...
\newglossaryentry{session id}{name=session ID,description={Session Identifier, a unique and secret string of text shared between a web server and a particular user, which is used by the server to identify this user}}
...
\newglossaryentry{ssl}{name=SSL,description={Secure Socket Layer, used in \gls{https} to encrypt traffic between a client and a server. The current version of SSL is known as TLS, or Transport Layer Security}}
...
\newglossaryentry{web application framework}{name=web application framework,description={A framework providing the core functionality of a web application, which provides web developers with a foundation when building web applications}}
...
\newglossaryentry{url}{name=URL,description={Uniform Resource Locator, a string that specifies the location of a resource, such as a web page}}
