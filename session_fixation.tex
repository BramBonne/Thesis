...
%TODO: wetenschappelijke paper vermelden

Out of the methods for injecting a \gls{session id} described in section \ref{injecting-sid}, we consider \gls{xss} and \texttt{<meta>}-tag injection the most important. These injection attacks have a high severity rating \cite{Williams2010} and lots of websites are vulnerable \cite{Brown2010}. Ideally, these problems would be solved by finding a solution to XSS attacks. Unfortunately, as we saw in section \ref{xss-countermeasures}, solutions to XSS are often lacking. Other attack vectors, such as URL rewriting, subdomain cookie setting and response splitting are considered out of scope because either a client-side solution would be unable to distinguish between legitimate SIDs and forged SIDs, or because they exploit a bug at the browser or proxy level.
...
%TODO: mag hier de evaluatie uit ingediende paper gebruikt worden?
%To evaluate the add-on's performance, 

\begin{comment}
The user experience can also be negatively impacted by a
large overhead induced by client-side protection mechanisms.
We quantify this performance overhead by comparing the page
load times in Firefox with and without the add-on enabled.
We used a similar setup as for SessionShield [3], where the
network inconsistencies are eliminated by hosting all the pages
locally. A fake local DNS server and fake local web server
are used to serve the captured pages. Since the SessionShield
evaluation used wget, the loading of additional resources (e.g.
images, scripts) was irrelevant. In this experiment however,
Firefox will attempt to load these resources. For any request
except for the original page, the local web server consistently
responds with a 404 status code.
Using the ChickenFoot add-on [19], we start a timer, instruct
Firefox to load a page, and stop the timer. The experiment is
repeated three times, both with and without the add-on. The
average page-load times for the top 1000 pages (as reported by
Alexa.com) without session-fixation protection is 195.407
ms. When our add-on is enabled, the overhead is 198.242 ms.
The overhead induced by our countermeasure is approximately
3 milliseconds which is negligible when compared to average
loading times on the Internet, that are around 0.5 seconds [20].
\end{comment}
...
HttpOnly cookies, as described in section \ref{httponly}, are cookies that are only accessible by HTTP. A reader may assume that setting a cookie using the \texttt{HttpOnly} flag would prevent an attacker from using XSS to inject another value for this cookie. %Hier iets over dat attakcer de eerste kan zijn.

%NALEZEN: Even if the web server is able to set the cookie before the attacker does, this attacker is often still able to set a duplicate cookie for the parent domain. Indeed, as we described in section \ref{subdomain-setting}, an attacker can set a cookie with the same name as the HttpOnly cookie for the parent domain. This causes the browser to send both the parent domain cookie and the subdomain cookie when a page on the subdomain is accessed, while the server has no way of distinguishing between the two cookies.

\subsection{SessionShield}\label{sessionshield}
% Hoe lost dit javascript cookies op?
