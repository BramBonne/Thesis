...
In this section, we compare our solution to some of the countermeasures from the previous chapter. Afterwards, we give an overview of the countermeasures, and which scenarios they protect against. %TODO

\subsection{Renewing the session identifier}

Renewing the session identifier was mooted in the previous session to be the best solution against session fixation attacks. Unfortunately, this solution can only be implemented at the server side. Because of this, Johns et al. suggest that their proxy protecting against session fixation could also be implemented at the client side \cite{Johns2011}. It would then be expected that the proxy makes sure that an attacker can not take over a victim's session, since he does not know the victim's PSID. Unfortunately, this is not the case. To see why, consider the scenario depicted in Figure \ref{fig:johns-clientside}.

\begin{figure}[htb]
	\centering
	\includegraphics[width=\textwidth]{img/johns-attack.png}
	\caption[Attack on a client-side implementation of Johns et al.]{The attack on a client-side implementation of the proxy proposed by Johns et al.}
	\label{fig:johns-clientside}
\end{figure}


% Hier Johns et al. vermelden
% Stellen voor om proxy ook aan client-side toe te passen in paper, gaat niet zie onze paper. Vermelden dat ons ding basically hetzelfde werkt als dat van hun wanneer je dat aan de client-side zou zetten.
% Configureren van elke server is niet haalbaar voor client-side solution, dus: heuristieken (wordt ook in de paper voorgesteld)

\subsection{HttpOnly}\label{httponlyremark}

%TODO: verwijzen naar \ref{httponly}
% Onze oplossing is het 'duale' van HttpOnly: doet ook scheiding JavaScript & HTTP, maar dan omgekeerd
% Onze oplossing werkt AUTOMATISCH
...
Similarly to our solution, SessionShield does allow untrusted cookies to be accessed via untrusted channels afterwards. This is because SessionShield only intercepts HTTP traffic, and does not notice JavaScript and \texttt{<meta>} cookies being set. This allows JavaScript cookies to keep functioning correctly.
...
% Ook proberend XSS aanvallen via hijacking tegen te gaan. Verschil is dat onze aanpak volledig geautomatiseerd werkt en in plaats van stelen via JS probeert tegen te gaan (hetgeen bijna niet haalbaar is owv verschillende attack vectors), effectief de SID onbeschikbaar maakt.
